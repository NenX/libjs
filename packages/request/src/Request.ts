...
import {
  IRequest_AxiosRequestConfig,
  IRequest_ResponseError,
  IRequest_SpawnConfig,
  TRequest_PreReq,
  TRequest_SimpleReq,
} from './types';
import {
...
export class Request extends EventEmitter<{
  ...
  static genErrMsg(res?: AxiosResponse<any>, error?: AxiosError) {
    ...
    const rawMessage =
      error?.message === 'Network Error' ? '网络连接异常   ' : '';
    const msg: string =
      Request.getMsg(res) ?? codeMessage[status] ?? rawMessage ?? '未知错误';
    return msg;
  }
  static createErr(
  ...
  static getMsg(
    ...
    const {
      ...
      successText,
      errText,
    } = res?.config as IRequest_AxiosRequestConfig;
  ...
  static getHeaderMessage(res?: AxiosResponse<any>) {
    ...
    const headerMessage = headerMessageRaw
      ? decodeURIComponent(headerMessageRaw)
      : undefined;
  ...
  gen(spawnConfig: IRequest_SpawnConfig = {}) {
    const {
      config = {} as IRequest_AxiosRequestConfig,
